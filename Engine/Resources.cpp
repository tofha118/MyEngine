...
void Resources::CreateDefaultShader()
{
	...
	// Deferred (WireFrame)
	{
		...
		{
			SHADER_TYPE::DEFERRED,
			RASTERIZER_TYPE::WIREFRAME,  //와이어 프레임으로 바꿔줌
	...
	// DirLight
	{
		ShaderInfo info =
		{
			...
			RASTERIZER_TYPE::CULL_NONE,  //순서 상관없이 그려달라
			DEPTH_STENCIL_TYPE::NO_DEPTH_TEST_NO_WRITE,
	...
	// Particle
	{
		ShaderInfo info =
		{
			...
			D3D_PRIMITIVE_TOPOLOGY_POINTLIST  //항상 삼각형인거 아니니까 받아줘서 넘기기
		};
	...
	// Tessellation
	{
		...
		{
			SHADER_TYPE::FORWARD,
			RASTERIZER_TYPE::WIREFRAME,  //각 삼각형의 변만 색상으로 보여줌.
			DEPTH_STENCIL_TYPE::LESS, 
			BLEND_TYPE::DEFAULT,
			D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST  //3개짜리로 이루어진 물체다
		};
