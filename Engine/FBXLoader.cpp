...
void FBXLoader::Import(const wstring& path)
{
	...
	// 씬 내에서 삼각형화 할 수 있는 모든 노드를 삼각형화 시킨다. 우리가 삼각형으로 하기때문에 추가
...
void FBXLoader::LoadMesh(FbxMesh* mesh)
{
	...
	assert(polygonSize == 3);  //아까 삼각형으로 해줬으니까 여기 안걸림
		...
		if (fbxSkin)
		{
			...
			if (FbxSkin::eRigid == type || FbxSkin::eLinear)
			{
				...
				for (int32 j = 0; j < clusterCount; j++)
				{
					...
					LoadBoneWeight(cluster, boneIdx, meshInfo);  //하나의 정점이 여러개의 관절의 영향을 받을 수있음. 그래서 어느정도의 영향을 받는지를 추출해주는거임.
