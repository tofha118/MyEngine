...
void Engine::Update()
{
	...
	GET_SINGLE(InstancingManager)->ClearBuffer();  //업데이트 할때마다 인스턴스 버퍼 한번씩 밀어줌.  (데이터를 밀어주겠다는거임. 버퍼를 새로만들겠다는게 아니라
...
void Engine::Render()
{
	...
	GET_SINGLE(ImguiManager)->Render_Before();
...
void Engine::ResizeWindow(int32 width,int32 height)
{
	...
	//사이즈가 변경되면 깊이버퍼도 사이즈 바꿔야 하기 때문엥
...
void Engine::CreateRenderTargetGroups()
{
	...
	// SwapChain Group
	{
		...
		//스왑체인의 전면버퍼와 후면버퍼를 꺼내옴
		for (uint32 i = 0; i < SWAP_CHAIN_BUFFER_COUNT; ++i)
		{
			wstring name = L"SwapChainTarget_" + std::to_wstring(i);

			ComPtr<ID3D12Resource> resource;
			//이미 존재하는 리소스를 통해서 만들어주기
	...
	// Shadow Group
	{
		...
		rtVec[0].target = GET_SINGLE(Resources)->CreateTexture(L"ShadowTarget",
			DXGI_FORMAT_R32_FLOAT, 4096, 4096,  //크기가 4096으로 만들어졌다는게 다름
			CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
	...
	// Deferred Group
	{
		...
		rtVec[0].target = GET_SINGLE(Resources)->CreateTexture(L"PositionTarget",
			...
			CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
			D3D12_HEAP_FLAG_NONE, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET);  //렌더 타겟 용도로 켜줄거다
