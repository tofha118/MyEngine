...
//	ComPtr<ID3D12Resource> GetRenderTarget(int32 index) { return _rtvBuffer[index]; }
	...
	//지금 백버퍼,프론트 구분하는 인덱스 뱉음
	//uint32 GetCurrentBackBufferIndex() { return _backBufferIndex; }
	//얘는 지금 내가 활용하는 버퍼 자체를 뱉음 (현재 백버퍼가 무엇인지를 건내주는거임)
...
//	ComPtr<ID3D12Resource>	_rtvBuffer[SWAP_CHAIN_BUFFER_COUNT];  //버퍼 두개 사용할거니까 2개 (그릴 대상)
	uint32					_backBufferIndex = 0;  //얘는 백버퍼가 누구인지 추적해주는 기능임. (back,front) 구분


	//ComPtr<ID3D12DescriptorHeap>	_rtvHeap;
	//D3D12_CPU_DESCRIPTOR_HANDLE		_rtvHandle[SWAP_CHAIN_BUFFER_COUNT]; //이제 여기서 버퍼 안만들어
