...
void Shader::CreateGraphicsShader(const wstring& path, ShaderInfo info, ShaderArg arg)
{
	...
	//	CreateVertexShader(path, "VS_Main", "vs_5_0");  //버텍스쉐이더
	//	CreatePixelShader(path, "PS_Main", "ps_5_0");  //픽셀 쉐이더
	...
	D3D12_INPUT_ELEMENT_DESC desc[] =
	{
		...
		//인풋 슬롯도 바뀜. 앞 뒤 0,1
		//행렬 넘겨주고싶어서 float4 짜리 4개를 넘겨주느거임
	...
	_graphicsPipelineDesc.pRootSignature = GRAPHICS_ROOT_SIGNATURE.Get();  //서명 꺼내와서 연결
	...
	//_pipelineDesc.DepthStencilState.DepthEnable = FALSE;  //여기가 이제 깊이버퍼임.
	//_pipelineDesc.DepthStencilState.StencilEnable = FALSE;
	_graphicsPipelineDesc.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);  //깊이 버퍼 (기본값으로 초기화 , 깊이버퍼만 사용함. 스텐실 사용 x)
	_graphicsPipelineDesc.SampleMask = UINT_MAX;
	_graphicsPipelineDesc.PrimitiveTopologyType = GetTopologyType(info.topology);//info.topologyType;// D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
	...
	_graphicsPipelineDesc.DSVFormat = DXGI_FORMAT_D32_FLOAT;//GEngine->GetDepthStencilBuffer()->GetDSVFormat(); //깊이버퍼를 사용하면 포맷도 넣어줘야함.
	...
	case RASTERIZER_TYPE::CULL_NONE:
		...
		break;
	case RASTERIZER_TYPE::WIREFRAME:  //안에 프레임이 아니라 밖에 외곽선이 보이는거임.
...
void Shader::CreateComputeShader(const wstring& path, const string& name, const string& version)
{
	...
	_computePipelineDesc.pRootSignature = COMPUTE_ROOT_SIGNATURE.Get();  //루트시그니쳐를 이렇게 선언 해줘야 함.
...
void Shader::Update()
{
	//업데이트 할때에도 쉐이더 타입에 따라서 세팅 해줘야겠징
	if (GetShaderType() == SHADER_TYPE::COMPUTE)
		COMPUTE_CMD_LIST->SetPipelineState(_pipelineState.Get());
	else
	{
		GRAPHICS_CMD_LIST->IASetPrimitiveTopology(_info.topology);  //삼각형인지 아닌지 여기서 업데이트 하면서 판별
